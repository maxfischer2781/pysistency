...
def format_times(start, stop):
    ...
    if not deltas[2]:
        return 'elapsed: %4s, pcpu: ---s' % format_seconds(deltas[2])
    return 'elapsed: %4s, pcpu: %d%%' % (format_seconds(deltas[2]), 100.0 * sum(deltas[:-1]) / deltas[2])
...
class TimingTextTestResult(unittest.TextTestResult):
    ...
    def addSuccess(self, test):
        ...
        if self.showAll:
            self.stream.writeln("ok\t(%s)" % format_times(self._timings[test][0], stop_times))
        elif self.dots:
            self.stream.write('.')
            self.stream.flush()
